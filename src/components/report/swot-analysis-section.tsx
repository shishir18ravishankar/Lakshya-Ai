...
interface SwotAnalysisSectionProps {
  swot?: {
    strengths: Tagged<string>[];
    weaknesses: Tagged<string>[];
    // Not yet produced by the AI pipeline — see risks -> threats mapping,
    // pending as a follow-up. Left plain/optional so this section still
    // renders sensibly (empty) until that's wired in.
            ...
            <div className="p-5 rounded-xl border border-emerald-200 bg-emerald-50/30 space-y-3">
              ...
              <ul className="space-y-2 pt-1">
                {strengths.map((item, idx) => (
                  <li key={idx} className="flex items-start gap-2 text-xs text-slate-800 flex-wrap">
                    <CheckCircle2 className="h-3.5 w-3.5 text-emerald-600 mt-0.5 shrink-0" />
                    <span className="leading-relaxed flex-1 min-w-0">{item.value}</span>
