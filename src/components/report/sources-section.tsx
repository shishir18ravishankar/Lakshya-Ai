...
// FeasibilityResponse["sources"] entries aren't Tagged<T> — they're built
// server-side only from grounding_ids that survived tag enforcement, so
// every entry here is verified by construction. No per-item tag/badge to
// derive; the section-level "verified" badge below already covers it.
type SourceRow = FeasibilityResponse["sources"][number];
      ...
      <div className="rounded-xl border border-slate-200 bg-white p-6 sm:p-7 shadow-sm space-y-6">
        ...
        {items.length === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {items.map((src, idx) => {
              const name = src.source || "Government / Institutional Source";
              const context = `${src.stat_name}: ${src.stat_value}${src.unit ? ` ${src.unit}` : ""}`;
