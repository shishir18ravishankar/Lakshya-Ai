...
export function MarketReachSection({ marketReach }: MarketReachSectionProps) {
  ...
  const segments = marketReach.customerSegments || marketReach.targetSegments || [];
            ...
            {drivers.length > 0 ? (
              <ul className="space-y-2.5">
                {drivers.map((driver, idx) => (
                  <li
                    key={idx}
                    className="flex items-start gap-2.5 text-xs text-slate-700 p-2.5 rounded-lg bg-slate-50/70 border border-slate-200/70"
                  >
          ...
          <div className="lg:col-span-7 space-y-3">
            <div className="flex items-center justify-between">
              <h3 className="text-xs font-bold uppercase tracking-wider text-slate-700 flex items-center gap-1.5">
                ...
                <span>Target Customer Segments & Volume Share</span>
              </h3>
              <span className="text-[10px] text-slate-500 font-mono">100% Volume Allocation</span>
            </div>
                ...
                {segments.map((seg, idx) => (
                  ...
                  >
                    <div className="flex items-center justify-between gap-2">
                      ...
                      <span className="text-xs font-bold text-blue-800 bg-blue-50 px-2 py-0.5 rounded border border-blue-200 shrink-0">
                        {seg.sharePct}% share
                      </span>
                    </div>
